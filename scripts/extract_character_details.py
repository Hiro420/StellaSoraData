...
def main(character_id: int = TARGET_CHARACTER_ID) -> None:
    ...
    skill_lang = load_json(
        BASE_DIR / "JP" / "language" / "ja_JP" / "Skill.json")
    ...
    for field_name, skill_id in skill_fields.items():
        if isinstance(skill_id, int):
            output_skills[field_name] = extract_skill_details(
                skill_id, skill_data, skill_lang)
        else:
    ...
    output["SkillUpgradeDetails"] = extract_upgrade_details(
        skills_upgrade_group, skill_upgrade_data)
